...
import com.github.bandwidthlimiter.BandwidthLimiters
...
class GenericCellRateRefillStrategySpecification extends Specification {
    ...
    def "Specification for initialization"() {
        setup:
            ...
            def builder = BandwidthLimiters.leakyBucketWithCustomPrecisionPrecision(metter)
    ...
    def "Specification for refill"() {
        setup:
            ...
            def builder = BandwidthLimiters.leakyBucketWithCustomPrecisionPrecision(meter)
                    .withGenericCellRateRefillStrategy()
    ...
    private long size(LeakyBucketState state, Bandwidth bandwidth) {
        return state.getCurrentSize(bandwidth)
    }

    private long refillTime(LeakyBucketConfiguration configuration, LeakyBucketState state, Bandwidth bandwidth) {
