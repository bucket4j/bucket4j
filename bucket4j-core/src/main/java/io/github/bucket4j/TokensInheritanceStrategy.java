...
public enum TokensInheritanceStrategy {
    ...
    /**
     * Instructs to copy available tokens as is, but with one exclusion: if available tokens are greater than new capacity,
     ...
     * <p> <b>Example 2:</b> imagine bandwidth that was created by {@code Bandwidth.builder().capacity(100).refillGreedy(10, ofMinutes(1)).build()}.
     * At the moment of config replacement it was 40 available tokens. After replacing this bandwidth by following {@code Bandwidth.builder().capacity(20).refillGreedy(10, ofMinutes(1)).build()}
     * 40 available tokens cannot be copied as is, because they are greater than new capacity, so available tokens will be reduced to 20.
     */
    AS_IS((byte) 1),
    ...
    private static final TokensInheritanceStrategy[] modes = new TokensInheritanceStrategy[] {
            PROPORTIONALLY, AS_IS, RESET, ADDITIVE
    };
