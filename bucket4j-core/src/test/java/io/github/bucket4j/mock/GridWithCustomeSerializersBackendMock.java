...
public class GridWithCustomeSerializersBackendMock<K extends Serializable> extends AbstractBackend<K> {
    ...
    public GridWithCustomeSerializersBackendMock(TimeMeter timeMeter) {
    ...
    public <T extends Serializable> CommandResult<T> execute(K key, RemoteCommand<T> command) {
        ...
        MutableBucketEntry entry = new MutableBucketEntry() {
            ...
            public void set(RemoteBucketState state) {
                GridWithCustomeSerializersBackendMock.this.stateMap.put(key, emulateDataSerialization(state));
            }
            @Override
    ...
    private static <T> T emulateKeySerialization(T object) {
        if (object == null) {
            return null;
        }
        try {
