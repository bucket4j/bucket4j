...
class DelayedAsyncCommandExecutorSpecification extends Specification {
    ...
    def "Should delay async consumption"() {
        ...
        then: "request propogated to proxyManager"
            ...
            notOptimizedBucket.getAvailableTokens().get() == 34

        when: "too many optimized bucket overconsumed the bucket"
            List<AsyncBucketProxy> buckets = new ArrayList<>()
            for (int i = 0; i < 10; i++) {
                buckets.add(proxyManager.builder().withSynchronization(synchronization).build(1L, {CompletableFuture.completedFuture(configuration)}))
            }
            for (int i = 0; i < 10; i++) {
        ...
        then: "amount of token in the proxyManager become negative"
            for (int i = 0; i < 10; i++) {
                buckets.get(i).tryConsume(1).get() == false
            }
            notOptimizedBucket.getAvailableTokens().get() == -167
    }
