...
The `VerboseResult` API can be used to retrieve the current `BucketConfiguration` of a bucket, as well as low-level diagnostic  which can be used to include an HTTP headers that describes the current rate limit.
