 ...
 */
public class CoherenceProxyManager<K extends Serializable> implements ProxyManager<K> {

    private final GridProxy<K> gridProxy;

    CoherenceProxyManager(NamedCache<K, GridBucketState> cache) {
        if (cache == null) {
            throw new IllegalArgumentException("cache must not be null");
        }
        this.gridProxy = new CoherenceProxy<>(cache);
    }
